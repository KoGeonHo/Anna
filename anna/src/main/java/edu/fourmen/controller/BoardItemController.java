...
<<<<<<< HEAD
	private static final Logger logger = LoggerFactory.getLogger(BoardItemController.class);
=======
>>>>>>> parent of 7248137 (Merge branch 'master' into seongmin)
	@Autowired
...
<<<<<<< HEAD
	public String itemlist(BoardItemVO vo, Criteria criteria, PageMaker pm, HttpServletRequest request, Model model) {
		 ...
		 * 
		 * // 보여줄 페이지의 시작 ROWNUM - 0부터 시작 int startPage = 0 + (pagenumber - 1) *
		 * pagecount; // 보여줄 페이지의 끝 ROWNUM int endPage = pagenumber * pagecount;
		 * 
		 ...
		 * 
		 * // 전체 페이지 갯수 구하기 int totalPageCount = (int) Math.ceil(totalRow / (double)
		 * pagecount);
		 * 
	...
	public String selectitem(int item_idx, HttpServletResponse response, HttpServletRequest request,
=======
